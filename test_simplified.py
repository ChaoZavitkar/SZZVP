...
with app.app_context():
    ...
    result = db_neo4j.run('''
        ...
        WITH other, collect(user) as user_likes
        WHERE length(user_likes) = 0
    ...
    expected = sorted(['bob@test.com', 'jiri@test.com'])
    actual = sorted([r['email'] for r in result])

    if actual == expected:
        print("   ✅ Správně: nevidí Boba (mutual), vidí Jiřího")
    else:
    ...
    result = db_neo4j.run('''
        ...
        MATCH (alice)-[:LIKES|:LIKES]-(other:User)
        WHERE other.id <> alice.id

        OPTIONAL MATCH (alice)-[:LIKES]->(other) as i_like
        OPTIONAL MATCH (other)-[:LIKES]->(alice) as they_like

        WITH other,
             CASE WHEN i_like IS NOT NULL AND they_like IS NOT NULL THEN true ELSE false END as is_match
